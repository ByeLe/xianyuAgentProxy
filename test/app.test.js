...
test('BotmuxClient 可从 relay bot 会话文件定位 session', async () => {
  ...
  try {
    ...
    assert.equal(found.agentFrozen, false);
...
test('BotmuxClient 可从 target bot 会话文件定位 session 与冻结状态', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'xianyu-agent-proxy-'));
  const sessionsPath = join(dir, 'sessions-cli_target.json');
  await writeFile(sessionsPath, JSON.stringify({
    frozen: {
      sessionId: 'frozen_target',
      agentFrozen: true,
      updatedAt: '2026-07-12T00:00:00.000Z',
      lastUserPrompt: 'conversation_key: xianyu:cid_old:buyer_old'
    },
    active: {
      sessionId: 'active_target',
      agentFrozen: false,
      rootMessageId: 'om_target_root',
      updatedAt: '2026-07-12T00:02:00.000Z',
      lastUserPrompt: '【闲鱼买家消息】\nconversation_key: xianyu:cid_1:buyer_1'
    }
  }));

  const client = new BotmuxClient({
    command: 'botmux',
    targetSessionsPath: sessionsPath
  });

  try {
    const found = await client.waitForTargetSession({
      conversationKey: 'xianyu:cid_1:buyer_1',
      timeoutMs: 1,
      intervalMs: 1
    });
    const frozen = await client.findTargetSessionById('frozen_target');

    assert.equal(found.sessionId, 'active_target');
    assert.equal(found.rootMessageId, 'om_target_root');
    assert.equal(found.agentFrozen, false);
    assert.equal(frozen.agentFrozen, true);
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
...
test('闲鱼消息会被投递到话题群，并返回 correlation_id', async () => {
  ...
  try {
    ...
    assert.equal(calls[0].options.headers?.['x-botmux-async'], undefined);
...
test('首条消息会初始化 relay session，后续消息走 relay 唤醒目标机器人', async () => {
  ...
  const app = createApp({
    ...
    botmuxClient: {
      ...
      waitForTargetSession: async (input) => {
  ...
  try {
    ...
    assert.equal(webhookCalls[0].options.headers?.['x-botmux-async'], undefined);
    ...
    assert.equal(targetLookups.length, 1);
    assert.equal(targetLookups[0].conversationKey, 'xianyu:cid_1:buyer_1');
    ...
    assert.equal(first.body.target_session_source, 'target_session_store');
    assert.equal(first.body.target_agent_frozen, false);
