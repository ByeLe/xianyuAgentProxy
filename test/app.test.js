...
test('已有飞书话题锚点时，闲鱼消息会回复到原话题', async () => {
  ...
  try {
    ...
    assert.match(feishuCalls[0].text, /apiproxy_reply_url: http:\/\/proxy\.local\/agent\/reply/);
...
test('首条 webhook 返回 botmux session 后，会自动保存锚点并用飞书 API 投递买家消息', async () => {
  ...
  try {
    ...
    assert.match(feishuCalls[0].text, /message_kind: xianyu\.buyer_message/);
    assert.match(feishuCalls[0].text, /apiproxy_reply_url: http:\/\/proxy\.local\/agent\/reply/);
    assert.ok(!feishuCalls[0].text.includes('完成后必须调用 apiproxy_reply_url'));
    assert.match(feishuCalls[0].text, /第一条/);
  } finally {
    server.close();
  }
});
