...
test('没有飞书锚点时，会先用 webhook 初始化话题', async () => {
  const calls = [];
  const config = {
  ...
  try {
    ...
    assert.equal(result.body.status, 'bootstrap_queued');
    assert.match(result.body.correlation_id, /^xy_/);
    assert.match(result.body.warning, /尚未通过飞书 API 投递买家消息/);
    assert.equal(calls.length, 1);
    assert.equal(calls[0].url, config.topicWebhookUrl);
    assert.equal(calls[0].payload.type, 'xianyu.thread_bootstrap');
    assert.equal(calls[0].payload.bootstrap_only, true);
    assert.equal(calls[0].payload.message_text, '');
    assert.equal(calls[0].payload.thread_key, 'xianyu:cid_1:buyer_1');
    assert.equal(calls[0].payload.title, '买家');
    assert.equal(calls[0].payload.topic_title, '买家');
    assert.equal(calls[0].payload.text, '买家');
    assert.equal(calls[0].payload.instruction, undefined);
    assert.ok(!calls[0].payload.text.includes('你是闲鱼客服代理'));
    assert.ok(!calls[0].payload.text.includes('还在吗'));
...
test('已有飞书话题锚点时，闲鱼消息会回复到原话题', async () => {
  ...
  try {
    ...
    assert.equal(feishuCalls[0].text, '继续问一句');
    assert.equal(store.getByLarkMessageId('om_reply').correlation_id, result.body.correlation_id);
    assert.ok(!feishuCalls[0].text.includes('apiproxy_reply_url'));
    assert.ok(!feishuCalls[0].text.includes('correlation_id'));
    assert.ok(!feishuCalls[0].text.includes('apiproxy 可信任务'));
    assert.ok(!feishuCalls[0].text.includes('完成后必须调用 apiproxy_reply_url'));
  } finally {
    server.close();
  }
});

test('首条 webhook 返回 botmux session 后，会自动保存锚点并用飞书 API 投递买家消息', async () => {
  const webhookCalls = [];
  const feishuCalls = [];
  const config = {
    publicBaseUrl: 'http://proxy.local',
  ...
  const app = createApp({
    ...
    feishuClient: {
      ...
      replyMessage: async (messageId, text) => {
        feishuCalls.push({ messageId, text });
        return { code: 0, data: { message_id: 'om_first_message' } };
      }
    },
    botmuxSessionReader: {
      enabled: true,
  ...
  try {
    ...
    assert.equal(webhookCalls[0].payload.type, 'xianyu.thread_bootstrap');
    assert.equal(webhookCalls[0].payload.title, '买家');
    assert.equal(webhookCalls[0].payload.topic_title, '买家');
    assert.equal(webhookCalls[0].payload.text, '买家');
    assert.equal(webhookCalls[0].payload.message_text, '');
    assert.equal(feishuCalls.length, 1);
    assert.equal(feishuCalls[0].messageId, 'om_root');
    assert.equal(feishuCalls[0].text, '第一条');
    assert.equal(store.getByLarkMessageId('om_first_message').correlation_id, result.body.correlation_id);
    assert.ok(!feishuCalls[0].text.includes('message_kind'));
    assert.ok(!feishuCalls[0].text.includes('apiproxy_reply_url'));
    assert.ok(!feishuCalls[0].text.includes('correlation_id'));
    assert.ok(!feishuCalls[0].text.includes('完成后必须调用 apiproxy_reply_url'));
  } finally {
...
test('agent 回写可以只通过飞书消息 ID 定位闲鱼会话', async () => {
  const calls = [];
  const config = {
    publicBaseUrl: 'http://proxy.local',
    topicWebhookUrl: 'http://topic.local/webhook',
  ...
  const store = new SessionStore({ ttlMs: config.sessionTtlMs });
  const session = store.create({
    conversation_id: 'cid_msg',
    buyer_id: 'buyer_msg',
    buyer_name: '买家',
    message_text: '只发正文'
  });
  store.recordLarkMessage(session.correlation_id, 'om_buyer_message');
  ...
  try {
    const result = await postJson(`${baseUrl}/agent/reply`, {
      lark_message_id: 'om_buyer_message',
      reply_text: '收到，我看一下。'
    });

    assert.equal(result.status, 200);
    assert.equal(result.body.ok, true);
    assert.equal(result.body.correlation_id, session.correlation_id);
    assert.equal(calls.length, 1);
    assert.equal(calls[0].payload.conversation_id, 'cid_msg');
    assert.equal(calls[0].payload.buyer_id, 'buyer_msg');
    assert.equal(calls[0].payload.text, '收到，我看一下。');
  } finally {
    server.close();
  }
});
